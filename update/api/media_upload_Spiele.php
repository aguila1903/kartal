...
if (isset($_REQUEST["spiel_id"])) {
    if ($_REQUEST["spiel_id"] != "null" && $_REQUEST["spiel_id"] != "") {
...
if ($bild_art == 'fr') {
    if (count($tmp_name_array) > 1) {
        $result = json_encode('Bei Eintrittskarten kann nur ein Bild hochgeladen werden!');
...
for ($i = 0; $i < count($tmp_name_array); $i++) {
    ...
    if (move_uploaded_file(utf8_decode($tmp_name_array[$i]), $path2 . utf8_decode($ref . "_" .$spiel_id. $name_array[$i]))) {
        $target = $path2 . utf8_decode($ref . "_" .$spiel_id. $name_array[$i]);   
        /*
         * ************** Front-Thumbnail Upload ********************************
         */
        if ($bild_art == 'ga') {
            list($wo, $ho) = getimagesize($target);
            $new = $path3 . utf8_decode($ref . "_" .$spiel_id. $name_array[$i]); // Pfad für den Thumbnail
        ...
        if ($bild_art == 'fr') {
            list($wo, $ho) = getimagesize($target);
            $new = $path3 . utf8_decode($ref . "_" .$spiel_id. $name_array[$i]); // Pfad für den Cover
            $type = substr($type_array[$i],-3);
            if ($wo < $ho) {
                $w = ($wo / $ho) * 400;
                $h = 400;
        ...
        if ($bild_art == 'ga') {
            list($wo, $ho) = getimagesize($target);
            $new = $path4 . utf8_decode($ref . "_" .$spiel_id. $name_array[$i]); // Pfad für den Cover
            $type = substr($type_array[$i],-3);
            if ($wo < $ho) {
                $w = ($wo / $ho) * 500;
                $h = 500;
        ...
        $querySQL = " insert into media (ref, dateiname, id, art) Values(" . $dbSyb->Quote($ref)
                . ", " . $dbSyb->Quote(utf8_decode($ref . "_".$spiel_id . $name_array[$i]))
                . ", " . $spiel_id
