...
if (isset($_REQUEST["stadion"])) {
    $stadion = trim($_REQUEST["stadion"]);
    $stadion = sonderzeichen($stadion);
} else {
    $out{'response'}{'status'} = -1;
    $out{'response'}{'errors'} = array('errors' => "Stadion fehlt!");
...
if (isset($_REQUEST["ort"])) {
    ...
    $out{'response'}{'status'} = -1;
    $out{'response'}{'errors'} = array('errors' => "Ort fehlt!");
    print json_encode($out);
...
// echo count($imgs[0]); // 68
    for ($i = 0; $i < count($imgs[0]); $i++) {
        ...
        if (preg_match("/stadien\/detail/i", $imgs[0][$i])) {
...
get_image("http://www.weltfussball.de/spielorte/$stadion-$ort/", "spieler", $stadion, "st", "fr", $stadion_id);
?>