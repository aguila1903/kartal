...
if (!$dbSyb->IsConnected()) {
    ...
    $out{'response'}{'errors'} = array('stadionname' => utf8_encode($dbSyb->ErrorMsg()));
...
$sqlQuery = "call editSpieler("
        . $dbSyb->Quote(utf8_decode($spieler_id)) .
        ", " . $dbSyb->quote(utf8_decode($name)) .
        ", " . $dbSyb->quote(utf8_decode($vorname)) .
        ", " . $dbSyb->quote(utf8_decode($land));
if ($geb_datum == null) {
    $sqlQuery .= " ,NULL ";
} else {
...
if (!$rs) {
    $out{'response'}{'status'} = -4;
    $out{'response'}{'errors'} = array('spieler_id' => utf8_encode($dbSyb->ErrorMsg()));
...
If (isset($rs->fields{'ergebnis'})) {
    if ($rs->fields{'ergebnis'} == 1) {
        ...
        $out{'response'}{'status'} = -4;
        $out{'response'}{'errors'} = array('spieler_id' => "Es wurden keine Änderungen vorgenommen. Entweder gab es keine Änderungen oder es ist ein Fehler aufgetreten. </br>" . utf8_encode($dbSyb->ErrorMsg()));

        print json_encode($out);
        return;
    } else if ($rs->fields{'ergebnis'} == 99) {
        $out{'response'}{'errors'} = array('spieler_id' => "Das von Ihnen eingegebene Land ist ungültig. Bitte prüfen Sie es erneut </br>" . utf8_encode($dbSyb->ErrorMsg()));

        print json_encode($out);
        return;
    } else if ($rs->fields{'ergebnis'} == 98) {
        $out{'response'}{'errors'} = array('trikot_nr' => "Diese Trikot-Nr ist in der Mannschaft bereits vergeben.</br>" . utf8_encode($dbSyb->ErrorMsg()));
        ...
        $out{'response'}{'errors'} = array('spieler_id' => "Dieser Spieler ist eventuell schon in der Liste. Wollen Sie dennoch fortfahren?</br>" . utf8_encode($dbSyb->ErrorMsg()));
        $out{'response'}{'data'} = $value;

        print json_encode($out);
    } else {
        $out{'response'}{'status'} = -4;
        $out{'response'}{'errors'} = array('spieler_id' => "Update konnte nicht durchgeführt werden. </br>" . utf8_encode($dbSyb->ErrorMsg()));
    ...
    $out{'response'}{'status'} = -4;
    $out{'response'}{'errors'} = array('spieler_id' => "Keine Ergebnis-Rückmeldung erhalten </br>" . utf8_encode($dbSyb->ErrorMsg()));
