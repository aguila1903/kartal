...
if (isset($_REQUEST["verein"])) {
    $verein = trim($_REQUEST["verein"]);
    $verein = sonderzeichen($verein);
} else {
...
// echo count($imgs[0]); // 68
    for ($i = 0; $i < count($imgs[0]); $i++) {
        ...
        if (preg_match("/wappen\/mittel/i", $imgs[0][$i])) {
...
// return;
    $suchmuster = "/((s.weltsport){1}.*(gif\"){1})/Us"; // Muss je nach Seite, von der das Bild runtergeladen wird, angepasst werden.
...
// return;
    ...
    $savenameGIF = utf8_decode($ref ."_".$id. "_" . $name . ".gif");

    $targetGIF = $path2 . $savenameGIF;
...
//    file_put_contents("image_scan.txt", $path1);
    ...
    gif2jpg($targetGIF, $path4 . $savename, 100);
    unlink($targetGIF);
    ...
    // list($wo, $ho) = getimagesize($targetGIF);
    ...
    // $h = ($ho / $wo) * 175;
    // }
    // resize($targetGIF, $new, $w, $h, $type); // Funktion in der reseizer.php wird ausgeführt
...
get_image("http://www.weltfussball.de/teams/$verein/", "spieler", $verein, "vn", "fr", $verein_id);
