...
for ($i = 0; $i < count($tmp_name_array); $i++) {
    ...
    if (move_uploaded_file(utf8_decode($tmp_name_array[$i]), $path2 . utf8_decode($ref ."_".$id. "_" . $name_array[$i]))) {
        $target = $path2 . utf8_decode($ref ."_".$id. "_" . $name_array[$i]);
        ...
        $new = $path3 . utf8_decode($ref ."_".$id. "_" . $name_array[$i]); // Pfad für den Thumbnail
        $type = $type_array[$i];
        ...
        if ($bild_art == 'fr') {
            list($wo, $ho) = getimagesize($target);
            $new = $path4 . utf8_decode($ref ."_".$id. "_" . $name_array[$i]); // Pfad für den Cover
            $type = $type_array[$i];
        ...
        $querySQL = " insert into media (ref, dateiname, id, art) Values(" . $dbSyb->Quote($ref)
                . ", " . $dbSyb->Quote(utf8_decode($ref ."_".$id. "_" . $name_array[$i]))
                . ", " . $id
