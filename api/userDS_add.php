...
//        }
        if (filter_var($e_mail, FILTER_SANITIZE_EMAIL) === false || filter_var($e_mail, FILTER_VALIDATE_EMAIL) === false) {
            errorMsg('Bitte eine gültige Email-Adresse eingeben!', 8);
        }
    } else {
...
$rs->Close();
if ($ergebnis == 1) {
    if (count_chars($e_mail) > 1) {
        sendMail($e_mail, $host, $uri, $userID, $benutzer);
    }
